...
# 可以直接修改下列參數進行傳輸測試
    ...
    .PARAMETER ${props['RDirectory']}
        $RDirectory = "D:\var\wrk\BHCF-10.10.133.1~2"
...
# Fuction to count local files.
{
    ...
    $Global:LocalFilesCount = (Get-ChildItem -Path "$RDirectory" -Force -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer -eq $false } |  Measure-Object).Count
...
try
{
    ...
    try
    {
        ...
        $synchronizationResult = $session.PutFiles(  "\\?\$RDirectory", "$LDirectory", 0 )
    }
    finally
    {
